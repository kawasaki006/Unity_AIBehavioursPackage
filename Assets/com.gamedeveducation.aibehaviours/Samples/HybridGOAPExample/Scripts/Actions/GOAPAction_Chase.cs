...
namespace HybridGOAPExample
{
    public class GOAPAction_Chase : GOAPAction_FSM
    {
        ...
        {
            // fixed cost for chasing is better
            return 20f;
        }
        ...
        Vector3 GetTargetLocation()
        {
            ...
            if (TargetGO != null)
            {
                ...
                // if chase is running, the object becomes the focused target, so mark it as the final target object
                LinkedBlackboard.Set(CommonCore.Names.Target_GameObject, TargetGO); 
            }
            else // attempt to get target position
            {
